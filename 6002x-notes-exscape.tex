...
Let's calculate the center frequency. This isn't quite as simple as taking the arithmetic mean $\displaystyle \frac{f_1 + f_2}{2}$, which would give us $\displaystyle \frac{1000 + 10000}{2} = 5500$ Hz. Instead, we need to take the \emph{geometric mean}, which is defined as $\sqrt[n]{a_1 \cdot a_2 \cdots a_{n-1} \cdot a_{n}}$ - that is, multiply the number together, and take the $n$th root of the product, where $n$ is the count of numbers. So, the center frequency we want is computed as
...
Now all that remains is to solve the system of equations we get for these two, setting our $\omega_0$ equal to the circuit equation, and doing the same for the bandwidth:
...
So, for $R = 680 \Omega$, the values are roughly $L = 12.025$ mH and $C = 210.65$ nF.
